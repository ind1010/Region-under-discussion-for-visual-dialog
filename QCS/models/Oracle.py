...
class Oracle(nn.Module):
    ...
    def forward(self, questions, obj_categories, spatials, focus, crop_features, visual_features, lengths):
        ...
        if self.inputs_config['focus']:
            mlp_in = torch.cat([mlp_in, focus],1)
        if self.inputs_config['visual']:
